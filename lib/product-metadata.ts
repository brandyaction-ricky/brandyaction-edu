...
export const productMetadataFields = ['thumbnail_url', 'detail_image_url', 'detail_images', 'regular_price', 'seo_title', 'seo_description', 'detail_html', 'detail_html_document', 'cta_price_label', 'cta_label', 'cta_url', 'cta_color', 'meta_pixel_id', 'recruitment_countdown_enabled'] as const;
...
export function mergeProductMetadata(previous: unknown, values: Record<string, unknown>) {
  ...
  for (const field of productMetadataFields) {
    ...
    if (field === 'recruitment_countdown_enabled') {
      if (typeof value !== 'boolean') throw new Error('카운트다운 표시 설정을 확인해 주세요.');
    ...
    } else if (field === 'cta_price_label' || field === 'cta_label' || field === 'cta_url' || field === 'cta_color' || field === 'meta_pixel_id') {
      if (value !== null && typeof value !== 'string') throw new Error('CTA·추적 설정 형식을 확인해 주세요.');
      const content = String(value || '').trim();
      if (field === 'cta_price_label' && content.length > 40) throw new Error('CTA 왼쪽 문구는 40자 이하로 입력해 주세요.');
