...
export function RecruitmentFunnel({ courses, cohorts, canSave = false }: { courses: FunnelCourse[]; cohorts: FunnelCohort[]; canSave?: boolean }) {
  ...
  return <AdminSection title="무료 교육 → 유료 교육" description="첫 모집 경로 준비" bordered>
    ...
    <p className="conversion-muted">모집 링크의 이동 버튼 클릭은 카톡방 관리에서 확인합니다. 무료 신청과 연결한 유료 기수의 신청 이후 구매는 ‘무료 신청·구매 연결’에서 확인합니다. 첫·앵콜 방송 및 교육 상세 이동 요청은 방송·교육 안내 링크에서 확인합니다. 별도 출석 체크는 사용하지 않습니다. 실제 시청 시간과 1차·앵콜 구매 전환 구분은 아직 연결하지 않았습니다. 연결되지 않은 단계는 0명으로 표시하지 않습니다.</p>
  </AdminSection>;
}
