...
export function RecruitmentCampaignLinks({period,view}:{period:string;view:string}) {
 ...
 return <section hidden={!['settings','performance'].includes(view)} aria-label="모집별 마케팅 캠페인 연결">
  <h3>이 모집의 광고·오가닉 캠페인</h3>
  <p>무료 상품이 같은 캠페인 중 이번 모집에 사용할 항목을 연결하세요. 같은 카톡방을 계속 써도 캠페인은 모집별로 구분합니다.</p>
  <div hidden={view!=='settings'}>
   {data&&<fieldset disabled={pending}><legend>연결할 캠페인 · 최대 20개</legend>
    {data.candidates.length?data.candidates.map(item=><label key={item.id} style={{display:'block',marginBottom:12}}><input type="checkbox" checked={ids.includes(item.id)} disabled={!item.available} onChange={e=>setIds(current=>e.target.checked?[...current,item.id]:current.filter(id=>id!==item.id))}/> {item.name} · {item.usesAds?'광고':'오가닉'} · {item.start} ~ {item.end}{!item.available?' · 다른 모집에 연결됨':''}</label>):<p>같은 무료 상품에 등록된 캠페인이 없습니다. 광고·웨비나 성과의 캠페인 설정에서 먼저 등록하세요.</p>}
  ...
  <p>캠페인별 방문·클릭·광고비·수동 실측과 아래 회원 신청 이후 실제 결제는 서로 다른 집계입니다. 합산하거나 광고 구매 귀속으로 해석하지 않습니다. 연결은 고객의 과거 유입 기록을 바꾸지 않습니다.</p>
 </section>;
}
