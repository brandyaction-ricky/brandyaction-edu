...
export function CampaignFreeClass({ course, config, resources = [] }: { course: Row; config: LandingConfig; resources?: ProductResource[] }) {
  ...
  return <LandingTracker config={trackingConfig}>
    ...
    <div className="free-body"><div className="campaign-layout"><div className="free-sheet">
        <section data-section="detail">{documentSource || detailHtml ? <ProductDetailHtml html={detailHtml} documentSource={documentSource} applicationCta={{ href: conversion.url, label: conversion.label, disabled: false, enrolled: false, conversionUrl: conversion.url }} /> : image ? <div className="detail-image-stack">{images.map((item, index) => <img className="detail-image" src={item.path} alt={item.alt || `${t(course,'title')} 상세 안내 ${index + 1}`} loading="lazy" decoding="async" key={item.path + index} />)}</div> : <div className="panel-body"><Cover course={course}/><div className="reading-copy mt24">{t(course,'description') || t(course,'summary')}</div></div>}</section>
