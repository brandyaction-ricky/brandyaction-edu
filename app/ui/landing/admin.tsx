...
import { AdminButton, AdminDrawer, AdminPage, AdminPageHeader, AdminSelect, AdminStatusBadge, AdminToast } from '@/app/ui/final/admin-system';
...
export function LandingAdmin() {
  ...
  }, [courseId, campaignId, period, filters, settingsOpen, sampleMin]);
  useEffect(() => {
    if (!dirty) return;
  ...
  const classify: Classify = async (row, value) => {
    ...
    if (okay) setResult(previous => !previous.report || previous.report.campaign.id !== campaign.id ? previous : ({ ...previous, report: { ...previous.report, performance: previous.report.performance.map(item => item.adset === row.adset && item.creative === row.creative ? { ...item, ad_type: value } : item) } }));
    return okay;
  };
  async function sync() {
  ...
  return <AdminPage width="wide" template="analytics" className="landing-admin tracking-admin">
    ...
    <section className="tracking-toolbar" aria-label="무료클래스 조회 조건">
      <div className="tracking-context-row"><AdminSelect label="무료클래스" value={courseId} onChange={event => chooseCourse(event.target.value)} disabled={pending || !courses.length}>{!courseId && <option value="">{list.loading ? '불러오는 중' : '클래스 선택'}</option>}{courses.map(item => <option key={item.id} value={item.id}>{item.title}{item.status === 'published' ? '' : ' · 미공개'}</option>)}</AdminSelect><AdminSelect label="캠페인" value={campaignId} onChange={event => chooseCampaign(event.target.value)} disabled={pending || !course?.campaigns.length}>{!campaignId && <option value="">등록된 캠페인 없음</option>}{course?.campaigns.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</AdminSelect><div className="tracking-context-status"><AdminStatusBadge status={stateKey} label={stateLabel}/>{campaign && <AdminStatusBadge status={campaign.meta_sync_status} label={metaStatus(campaign, syncing)} tone={campaign.meta_sync_status === 'failed' ? 'danger' : campaign.meta_sync_status === 'success' ? 'success' : undefined}/>}</div><span className="tracking-toolbar-spacer"/><AdminButton onClick={refresh} disabled={pending || loading}><RefreshCw size={16}/>새로고침</AdminButton><AdminButton tone="secondary" onClick={() => setSettingsOpen(true)} disabled={!campaign}><Settings2 size={16}/>캠페인 설정</AdminButton></div>
      ...
      <TrackingFiltersPanel options={report?.options} filters={filters} onChange={setFilters} exportUrl={`/api/landing/performance/export?${query}`} disabled={!query || loading || !!error}/></>}
    </section>
    ...
    <div className="tracking-main">
      ...
      {!campaign ? list.loading ? <TrackingSkeleton/> : <CompactEmpty title={courses.length ? '캠페인이 등록되지 않았습니다.' : '무료클래스가 등록되지 않았습니다.'} action={<Link className="admin-button admin-button--tertiary" href="/admin/products">상품 관리 확인</Link>}>무료클래스와 연결된 캠페인 구성을 확인해 주세요.</CompactEmpty> : !report ? error ? null : <TrackingSkeleton/> : <>
        {loading && <p className="tracking-help" role="status">새 조건을 조회하고 있습니다. 아래는 이전 조회 결과입니다.</p>}
        <div aria-busy={loading} className={loading ? 'tracking-refreshing' : ''}><PerformanceDashboard key={campaign.id} report={report} compare={period.compare} filtered={Object.values(filters).some(values => values.length > 0)} sampleMin={sampleMin} onSampleMin={setSampleMin} pending={pending || loading || !!error} onRetry={() => setReload(value => value + 1)} onClassify={classify}/><ActualsPanel report={report} onEdit={row => setDrawer({ row })} onAdd={() => setDrawer({ row: null })} onSettings={() => setSettingsOpen(true)} onRetry={() => setReload(value => value + 1)}/></div>
      </>}
    </div>
