...
function Editor({code,phase,session,offerReady,origin,saved}:{code:string;phase:LivePhase;session?:BroadcastSession;offerReady:boolean;origin:string;saved:()=>void}){
 ...
 return <fieldset disabled={pending}>
  ...
  {session&&(['paid','organic','unknown'] as const).map(channel=><div key={channel}>
   <AdminInput readOnly label={`${liveLabel(phase)} ${channel==='paid'?'광고방':channel==='organic'?'오가닉방':'공통'} 방송 링크`} value={`${origin}/go/${code}/${phase}/${channel}/live`}/>
   {offerReady&&<AdminInput readOnly label={`${liveLabel(phase)} ${channel==='paid'?'광고방':channel==='organic'?'오가닉방':'공통'} 교육 안내 링크`} value={`${origin}/go/${code}/${phase}/${channel}/offer`}/>}
  </div>)}
  {message&&<p role="alert">{message} 최신 설정을 다시 불러와 주세요.</p>}
 </fieldset>;
}
export function WebinarBroadcastManagement({code}:{code:string}){
 ...
 return <AdminSection title="방송·교육 안내 링크" description="카톡방 링크를 누르면 로그인·출석 체크 없이 방송 또는 교육 상세페이지로 바로 이동합니다." bordered>
  <p>광고방·오가닉방에는 해당 링크를, 양쪽이 함께 보는 방송 채팅에는 공통 링크를 사용하세요. 링크 종류는 안내 위치를 나타내며 실제 광고 유입이나 개인 신원을 증명하지 않습니다. 저장 후에도 카톡방 공지·광고에 자동 게시하지 않습니다.</p>
  {report&&(['first','encore'] as const).map(phase=><Editor key={`${phase}:${refresh}`} code={code} phase={phase} session={report.sessions.find(s=>s.phase===phase)} offerReady={report.offerReady} origin={origin} saved={reload}/>)}
  <h3>이동 요청 기록</h3>
  <p>출석·시청자·고유 인원이 아닙니다. 알려진 미리보기·사전 로딩 요청은 제외하지만 반복 방문과 일부 자동 요청이 포함될 수 있습니다. 분당 수집 한도 초과 시 이동은 허용하고 기록은 생략합니다. 고객 계정·IP·새 추적 쿠키는 저장하지 않습니다.</p>
  ...
  <p>YouTube 전체 시청 통계는 아직 연결하지 않았습니다. 기존 회원의 신청 이후 구매·환불 조회는 위 패널에 있으며, 이 이동 요청과 개인별로 연결하거나 첫·앵콜 구매 전환율로 계산하지 않습니다. 과거 출석 이력은 보존하며 추가 출석 체크는 사용하지 않습니다.</p>
