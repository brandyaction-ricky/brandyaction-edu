import { BookOpen, CheckSquare2, Code2, CreditCard, FileImage, FileText, GraduationCap, LayoutDashboard, Megaphone, MessageSquareText, MessagesSquare, Route, Settings, ShieldCheck, Tags, TicketPercent, Users, Target, ListChecks, Files, Search, UsersRound, ChartNoAxesColumn } from "lucide-react";
import { getAdminSession } from "@/lib/server-auth";
import { AdminThemeToggle } from "@/app/components/admin-theme-toggle";
...
export async function AdminShell({active,children}:{active:string;children:React.ReactNode}){
  ...
  return <div className="admin-app"><a className="admin-skip-link" href="#admin-main">본문으로 바로가기</a><AdminNavigation active={active} groups={navGroups.map(group=>({label:group.label,items:group.items.filter(item=>canSee(item.id)).map(({id,label,href})=>({id,label,href}))})).filter(group=>group.items.length)}/><div className="admin-body"><header className="admin-topbar"><div><span>운영센터</span><strong>브랜디액션 에듀</strong></div><div><AdminThemeToggle/><span className="user-avatar">{name[0]}</span><div><strong>{name}</strong><small>{isAdmin?"최고 관리자":"스태프"}</small></div></div></header><main className="admin-content" id="admin-main" tabIndex={-1}>{children}</main></div></div>
}
