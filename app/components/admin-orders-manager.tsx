...
import { Download, ExternalLink, Filter, RefreshCw, Search } from "lucide-react";
...
export function AdminOrdersManager() {
  ...
  const executeRefund = async () => {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({})) as { error?: string }; if (!response.ok) throw new Error(result.error || "환불하지 못했습니다."); setRefundTarget(null); await load();
  ...
  return <div className="order-ops-page">
    ...
    {detailTarget && (()=>{const payment=detailTarget.payments[0];const available=Math.max(0,(payment?.approved_amount||detailTarget.total_amount)-(payment?.cancelled_amount||0));return <div className="admin-modal-backdrop" onMouseDown={()=>setDetailTarget(null)}><section className="admin-modal order-detail-modal" role="dialog" aria-modal="true" aria-labelledby="order-detail-title" onMouseDown={(event)=>event.stopPropagation()}><header><div><span>ORDER DETAIL</span><h2 id="order-detail-title">주문 상세내역</h2><p>{detailTarget.order_number}</p></div><span className={`status-label ${detailTarget.status==="paid"?"success":detailTarget.status.includes("refund")?"refund":"planned"}`}>{label[detailTarget.status]||detailTarget.status}</span></header><section><h3>결제 정보</h3><dl><div><dt>주문자</dt><dd>{detailTarget.customer_name}<small>{detailTarget.customer_email}</small></dd></div><div><dt>결제 수단</dt><dd>{payment?.method||"미승인"}</dd></div><div><dt>결제 일시</dt><dd>{detailTarget.paid_at?date(detailTarget.paid_at):"결제 전"}</dd></div></dl></section><section><h3>상품 정보</h3>{detailTarget.order_items.map((item)=><article key={item.id}><div><strong>{one(item.courses)?.title||item.item_name}</strong><small>{one(item.cohorts)?.name||"기수 정보 없음"}</small></div></article>)}</section><section className="order-detail-amount"><h3>결제 금액</h3><dl><div><dt>승인 금액</dt><dd>{(payment?.approved_amount||detailTarget.total_amount).toLocaleString()}원</dd></div>{Boolean(payment?.cancelled_amount)&&<div><dt>취소 금액</dt><dd>-{payment.cancelled_amount.toLocaleString()}원</dd></div>}<div className="total"><dt>취소 가능 금액</dt><dd>{available.toLocaleString()}원</dd></div></dl></section><footer>{payment?.receipt_url&&<a className="admin-outline" href={payment.receipt_url} target="_blank" rel="noreferrer"><ExternalLink/>영수증</a>}<span/><button className="admin-outline" onClick={()=>setDetailTarget(null)}>닫기</button>{["paid","partially_refunded"].includes(detailTarget.status)&&available>0&&<button className="admin-primary" onClick={()=>{setDetailTarget(null);openRefund(detailTarget);}}>결제 취소</button>}</footer></section></div>})()}
