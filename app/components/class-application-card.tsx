...
export function ClassApplicationCard({ item }: { item: ClassItem }) {
  ...
  return <aside className="purchase-card">
    ...
    <dl>
      ...
      <div><dt>진행 방식</dt><dd>{digital ? "디지털 콘텐츠" : "온라인 클래스"}</dd></div>
      <div><dt>{digital ? "이용 안내" : "VOD·다시보기"}</dt><dd>{item.duration}</dd></div>
    </dl>
    {item.applicationOpen
      ? <Link className="button button-primary button-lg full" href={checkoutHref}>{digital ? "상품 구매하기" : "수강 신청하기"} <ArrowRight /></Link>
