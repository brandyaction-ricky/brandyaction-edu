...
import { Check, Edit3, Plus, RefreshCw, Save, Tag, Trash2, Users } from "lucide-react";
...
export function AdminMemberTagsManager(){
  const [data,setData]=useState<Data>(empty);const [draft,setDraft]=useState({...blank});const [loading,setLoading]=useState(true);const [saving,setSaving]=useState(false);const [error,setError]=useState("");const [message,setMessage]=useState("");
  ...
  return <div className="member-tags-page">
    ...
    <div className="member-tags-layout"><section className="admin-panel member-tag-create"><header><span>{draft.id?"태그 수정":"새 태그 등록"}</span><strong>분류 기준을 명확하게 기록하세요.</strong><p>누가 이 태그에 포함되는지 운영자가 같은 기준으로 판단할 수 있어야 합니다.</p></header><label>태그 색상<input type="color" value={draft.color} onChange={(event)=>setDraft({...draft,color:event.target.value})}/></label><label>태그 이름<input value={draft.name} onChange={(event)=>setDraft({...draft,name:event.target.value})} placeholder="예: 무료강의 참여" maxLength={40}/></label><label>분류 기준·활용 목적<textarea value={draft.description} onChange={(event)=>setDraft({...draft,description:event.target.value})} placeholder="예: 무료강의 2강 이상 시청한 회원 · 후속 유료 클래스 안내에 사용" maxLength={200}/></label><div>{draft.id&&<button className="admin-outline" onClick={()=>setDraft({...blank})}>취소</button>}<button className="admin-primary" onClick={()=>void save()} disabled={saving||!draft.name.trim()}><Save/>{draft.id?"수정 저장":"태그 등록"}</button></div></section>
    <section className="admin-panel member-tag-catalog"><header><div><h2>태그 목록</h2><p>사용 인원을 확인하고 회원 목록으로 바로 이동합니다.</p></div><button className="admin-outline" onClick={()=>setDraft({...blank})}><Plus/>새 태그</button></header><div className="member-tag-catalog-head"><span>태그</span><span>분류 기준</span><span>사용 회원</span><span>관리</span></div>{data.tags.map((tag)=>{const count=data.memberTags.filter((row)=>row.tag_id===tag.id).length;return <article key={tag.id} className={draft.id===tag.id?"editing":""}><div><i style={{background:tag.color}}/><strong>{tag.name}</strong></div><p>{tag.description||"분류 기준 미입력"}</p><Link href={`/admin/members?tag=${tag.id}`}>{count}명 보기</Link><div><button onClick={()=>setDraft({id:tag.id,name:tag.name,color:tag.color,description:tag.description||""})} aria-label={`${tag.name} 수정`}><Edit3/></button><button className="danger" onClick={()=>void remove(tag)} aria-label={`${tag.name} 삭제`}><Trash2/></button></div></article>;})}{!data.tags.length&&<div className="member-tag-empty"><Tag/><span><strong>등록된 태그가 없습니다.</strong><small>첫 태그를 등록하면 회원 관리와 CRM에서 사용할 수 있습니다.</small></span></div>}</section></div>
