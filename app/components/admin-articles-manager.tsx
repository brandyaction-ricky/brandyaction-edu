...
import { ArrowDown, ArrowLeft, ArrowUp, BookOpen, Check, ChevronRight, Eye, FileImage, FileText, GripVertical, ImagePlus, LoaderCircle, Pencil, Plus, Save, Search, Settings2, Star, Trash2, Upload } from "lucide-react";
...
function emptyArticle(categoryId = ""): Article {
  return { id: "", categoryId, categoryName: "", slug: "", title: "", summary: "", blocks: [{ id: crypto.randomUUID(), type: "paragraph", text: "" }], coverImagePath: "", coverImageUrl: "", coverImageAlt: "", status: "draft", isFeatured: false, seoTitle: "", seoDescription: "", scheduledAt: "", publishedAt: "", createdAt: "", updatedAt: "" };
}
...
export function AdminArticlesManager() {
  ...
  const load = async () => {
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : "아티클 데이터를 불러오지 못했습니다."); }
  ...
  const addCategory = async () => {
    ...
    if (!name) return;
    const ok = await mutate({ action: "saveCategory", category: { name, slug: slugify(name), displayOrder: data.categories.length, isActive: true } }, "카테고리를 추가했습니다.");
  ...
  const saveArticle = async (article = draft, success = "아티클을 저장했습니다.") => {
  ...
  const removeArticle = async (article: Article) => {
    if (!window.confirm(`‘${article.title}’ 아티클을 삭제할까요? 삭제 후 복구할 수 없습니다.`)) return;
    await mutate({ action: "deleteArticle", id: article.id }, "아티클을 삭제했습니다.");
  };

  if (loading) return <section className="admin-panel admin-loading-state"><LoaderCircle className="spin"/><strong>아티클 관리 데이터를 불러오는 중입니다.</strong></section>;
  return <>
    <div className="article-admin-nav"><button className={view === "list" ? "active" : ""} onClick={() => setView("list")}><FileText/>아티클 게시판</button><button className={view === "free-course" ? "active" : ""} onClick={() => setView("free-course")}><BookOpen/>무료강의 상단 설정</button><Link href="/articles" target="_blank"><Eye/>고객 화면 미리보기</Link></div>
    {error && <p className="admin-save-error">{error}</p>}
    {view === "list" && <ArticleList data={data} visible={visible} query={query} setQuery={setQuery} categoryFilter={categoryFilter} setCategoryFilter={setCategoryFilter} newCategory={newCategory} setNewCategory={setNewCategory} addCategory={addCategory} deleteCategory={deleteCategory} openNew={openNew} openEdit={openEdit} quickUpdate={quickUpdate} removeArticle={removeArticle} saving={saving}/>}
...
function ArticleList(props: { data: AdminPayload; visible: Article[]; query: string; setQuery: (value: string) => void; categoryFilter: string; setCategoryFilter: (value: string) => void; newCategory: string; setNewCategory: (value: string) => void; addCategory: () => void; deleteCategory: (category: ArticleCategory) => void; openNew: () => void; openEdit: (article: Article) => void; quickUpdate: (article: Article, changes: Partial<Article>, success: string) => void; removeArticle: (article: Article) => void; saving: boolean }) {
  return <div className="article-admin-workspace">
    <section className="admin-panel article-category-manager"><div className="article-admin-section-head"><div><span>01</span><div><h2>카테고리 관리</h2><p>고객 아티클 화면의 분류 탭과 섹션에 사용됩니다.</p></div></div><strong>{props.data.categories.length}개 카테고리</strong></div><div className="article-category-chips">{props.data.categories.map((category) => <span key={category.id}>{category.name}<button aria-label={`${category.name} 삭제`} onClick={() => props.deleteCategory(category)}>×</button></span>)}</div><div className="article-category-add"><input value={props.newCategory} onChange={(event) => props.setNewCategory(event.target.value)} onKeyDown={(event) => event.key === "Enter" && props.addCategory()} placeholder="새 카테고리 이름"/><button onClick={props.addCategory} disabled={props.saving}><Plus/>카테고리 추가</button></div></section>
    <section className="admin-panel article-board"><div className="article-board-head"><div><span>02</span><div><h2>아티클 게시판 <small>({props.data.articles.length})</small></h2><p>제목을 누르면 편집 화면으로 이동합니다.</p></div></div><button className="admin-primary" onClick={props.openNew}><Plus/>새 글 작성</button></div><div className="article-board-toolbar"><div className="article-filter-tabs"><button className={props.categoryFilter === "all" ? "active" : ""} onClick={() => props.setCategoryFilter("all")}>전체</button>{props.data.categories.map((category) => <button className={props.categoryFilter === category.id ? "active" : ""} key={category.id} onClick={() => props.setCategoryFilter(category.id)}>{category.name}</button>)}</div><label><Search/><input value={props.query} onChange={(event) => props.setQuery(event.target.value)} placeholder="제목·요약 검색"/></label></div>
      <div className="article-admin-table"><header><span>제목</span><span>카테고리</span><span>상태</span><span>작성일</span><span>관리</span></header>{props.visible.map((article) => <article key={article.id}><button className="article-title-cell" onClick={() => props.openEdit(article)}><strong>{article.isFeatured && <Star/>}{article.title}</strong><span>{article.summary || "요약이 없습니다."}</span></button><span className="article-category-pill">{article.categoryName}</span><span className={`article-status ${article.status}`}>{statusLabels[article.status]}</span><span>{dateLabel(article.publishedAt || article.updatedAt)}</span><div className="article-row-actions"><button className={article.isFeatured ? "featured" : ""} onClick={() => void props.quickUpdate(article, { isFeatured: true }, "대표 아티클로 지정했습니다.")}><Star/>대표</button><button onClick={() => void props.quickUpdate(article, { status: article.status === "published" ? "hidden" : "published" }, article.status === "published" ? "아티클을 숨겼습니다." : "아티클을 공개했습니다.")}>{article.status === "published" ? "숨김" : "공개"}</button><button onClick={() => props.openEdit(article)}><Pencil/>수정</button><button className="danger" onClick={() => props.removeArticle(article)}><Trash2/>삭제</button></div></article>)}{props.visible.length === 0 && <div className="article-admin-empty"><FileText/><strong>표시할 아티클이 없습니다.</strong><p>새 글을 작성하거나 검색 조건을 바꿔보세요.</p></div>}</div>
    </section>
  </div>;
}
