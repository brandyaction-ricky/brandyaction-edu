...
export function CheckoutClient({ course, initialCohortId, customer, clientKey, paymentMode, paymentReady, paymentConfigError }: {
  ...
  return <form className="checkout-wrap container" onSubmit={pay}><Link href={`/classes/${course.slug}`} className="back-link"><ArrowLeft size={18}/> 클래스 상세로</Link><div className="checkout-title"><span>ORDER</span><h1>수강 신청</h1><ol><li className="active">1 주문 확인</li><li>2 결제</li><li>3 신청 완료</li></ol></div><div className="checkout-layout"><div className="checkout-form"><section className="checkout-product-preview"><span className="ba-eyebrow">YOUR NEXT ACTION</span><h2>주문 상품</h2><strong>{course.title}</strong><p>{cohort.name} · {date(cohort.operationStartAt)}</p><span className="ba-badge">{cohort.price === 0 ? "무료 신청" : money(cohort.price)}</span></section>
