...
export async function GET(request: Request) {
  ...
  try {
    ...
    const headers = ['날짜','광고 유형','캠페인','광고세트','소재','기기','레이아웃 버전','방문','고유 방문자','CTA 클릭 세션','CTA 클릭 횟수','CTA 전환율','평균 스크롤','평균 체류','Meta 노출','Meta 링크 클릭','광고비','등록 결과 상태','Meta 등록완료','등록당 비용'];
    const registrationState = (row: Record<string, unknown>) => row.registration_available === true ? Number(row.registrations) === 0 ? '수집 · 0건' : '수집' : row.registration_available === false ? '미수집' : '기존 기록 · 확인 필요';
    const body = rows.map(row => [row.day,row.ad_type === 'cold'?'콜드':row.ad_type === 'retarget'?'리타겟':'미분류',displayDimension(String(row.campaign||'')),displayDimension(String(row.adset||'')),displayDimension(String(row.creative||'')),row.device,row.layout_ver,row.sessions,row.visitors,row.cta_click_sessions,row.cta_clicks,Number(row.sessions)>0?(Number(row.cta_click_sessions)/Number(row.sessions)*100).toFixed(1)+'%':'0%',row.avg_scroll_depth,row.avg_dwell_ms,row.impressions,row.link_clicks,row.spend,registrationState(row),row.registration_available === true ? row.registrations : row.registration_available === false ? '미수집' : '확인 필요',row.registration_available === true ? row.registration_cost : row.registration_available === false ? '미수집' : '확인 필요'].map(csvCell).join(','));
