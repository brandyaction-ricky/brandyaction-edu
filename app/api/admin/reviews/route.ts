...
export async function GET() {
  ...
  const { data, error } = await createAdminClient().from("reviews").select("id,author_name,rating,body,status,is_featured,created_at,profiles(email,phone),courses(title),cohorts(name)").order("created_at", { ascending: false });
...
export async function PATCH(request: Request) {
  ...
  if (typeof body.featured === "boolean") changes.is_featured = body.featured;
