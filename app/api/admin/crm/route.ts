...
export async function POST(request:Request){
  ...
  if(action==="saveTag"){
    ...
    let query=admin.from("profiles").select("id,full_name,phone").eq("status","active").eq("marketing_consent",true).not("phone","is",null).limit(500);if(memberIds)query=query.in("id",memberIds);
