...
import { createJevV3Judgment } from '@/lib/conversion-jev-v3';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingQuery = await db.from('edu_conversion_jev_v3_runs').select('id,status,result,updated_at').eq('v1_run_id', run.id).maybeSingle();
    ...
    if (existing) {
      ...
      if (existing.status !== 'failed' && !expired) conversionError('이 문의의 v3 판정이 진행 중입니다. 잠시 후 새로고침해 주세요.', 409);
      let renewal = db.from('edu_conversion_jev_v3_runs').update({ status: 'pending', result: null, actor_id: user.id, updated_at: new Date().toISOString() }).eq('id', existing.id).eq('status', existing.status);
      ...
      const claimed = await db.from('edu_conversion_jev_v3_runs').insert({ v1_run_id: run.id, case_id: run.case_id,
        calibration_review_id: first.id, input_version: run.input_version, status: 'pending', actor_id: user.id }).select('id').single();
      if (claimed.error?.code === '23505') conversionError('이 문의의 v3 판정이 진행 중입니다. 잠시 후 새로고침해 주세요.', 409);
      if (claimed.error) conversionDatabaseError(claimed.error);
      reservationId = claimed.data!.id;
    }

    const result = await createJevV3Judgment(run.input_snapshot.subject, run.input_snapshot.content, process.env.TYPESAFE_API_KEY || '');
    const saved = await db.from('edu_conversion_jev_v3_runs').update({ status: 'completed', result, updated_at: new Date().toISOString() })
    ...
    if (reservationId) await db.from('edu_conversion_jev_v3_runs').update({ status: 'failed', result: null, updated_at: new Date().toISOString() }).eq('id', reservationId).eq('status', 'pending');
    const status = error && typeof error === 'object' && 'status' in error ? Number(error.status) : 503;
    return reply({ error: error instanceof Error && status !== 503 ? error.message : 'Jev v3 판정을 완료하지 못했습니다. 잠시 후 다시 시도해 주세요.' }, status);
  }
}
