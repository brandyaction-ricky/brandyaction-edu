...
export default async function CohortHome({ params }: { params: Promise<{ enrollmentId: string }> }) {
  ...
  return <LearnerShell active="classes" userName={name}><div className="learning-home-heading"><div><span>{home.cohort.name}</span><h1>{home.course.title}</h1><p>{home.course.summary}</p></div><div><strong>{progress}%</strong><span>전체 진도 · {completed}/{lessons.length}</span></div></div>
    <section className="learning-live-list"><div className="panel-heading"><h2>라이브 일정·다시보기</h2><span>{home.sessions.length}회</span></div>{home.sessions.length ? home.sessions.map((session) => { const canEnter = canEnterLive(session.scheduledAt, session.liveUrl); return <article key={session.id}><span className="live-number">LIVE {session.number}</span><div><strong>{session.title}</strong><small><CalendarDays/>{dateTime(session.scheduledAt)} · {session.expectedOutput}</small></div><div className="live-row-actions">{canEnter && <a href={session.liveUrl!} target="_blank" rel="noreferrer"><Radio/> 라이브 입장</a>}{session.replayUrl && <a href={session.replayUrl} target="_blank" rel="noreferrer"><PlayCircle/> 다시보기</a>}{!canEnter && !session.replayUrl && <span><Clock3/> 수업 30분 전 오픈</span>}</div></article>; }) : <div className="learning-empty">등록된 라이브 일정이 없습니다.</div>}</section>
    ...
    <ReviewForm courseId={home.course.id} cohortId={home.cohort.id} authorName={name}/>
