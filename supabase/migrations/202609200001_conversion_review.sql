...
-- These records describe an operator review, never a customer reply or sale.
create table public.edu_conversion_cases (
  id uuid primary key default gen_random_uuid(),
  source_type text not null check (source_type in ('native','manual')),
  question_id uuid unique references public.edu_questions(id) on delete restrict,
  ...
  check ((source_type='native' and question_id is not null) or (source_type='manual' and question_id is null and customer_id is null))
);
create table public.edu_conversion_evidence (
  id uuid primary key default gen_random_uuid(),
  course_id uuid not null references public.courses(id) on delete restrict,
  cohort_id uuid references public.cohorts(id) on delete restrict,
  title text not null check (length(title) between 1 and 200),
  body text not null check (length(body) between 1 and 10000),
  source_url text not null check (length(source_url) between 1 and 2048 and source_url ~ '^https?://'),
...
create table public.edu_conversion_runs (
  id uuid primary key default gen_random_uuid(),
  ...
  result jsonb not null check (jsonb_typeof(result)='object' and result->>'mode'='mock'),
...
create table public.edu_conversion_reviews (
  id uuid primary key default gen_random_uuid(),
  case_id uuid not null references public.edu_conversion_cases(id) on delete restrict,
  run_id uuid not null references public.edu_conversion_runs(id) on delete restrict,
  decision text not null check (decision in ('accept','edit','hold','reject')),
  reply_text text not null default '' check (length(reply_text)<=10000),
  reason text not null default '' check (length(reason)<=1000),
  actor_id uuid references public.profiles(id) on delete set null,
  created_at timestamptz not null default now(),
  check (decision not in ('accept','edit') or length(btrim(reply_text))>0),
  check (decision='accept' or length(btrim(reason))>0)
);
create table public.edu_conversion_receipts (
...
create index edu_conversion_cases_created on public.edu_conversion_cases(created_at desc);
create index edu_conversion_evidence_course on public.edu_conversion_evidence(course_id,status);
create index edu_conversion_runs_case on public.edu_conversion_runs(case_id,created_at desc);
create index edu_conversion_reviews_case on public.edu_conversion_reviews(case_id,created_at desc);

alter table public.edu_conversion_cases enable row level security;
alter table public.edu_conversion_evidence enable row level security;
alter table public.edu_conversion_runs enable row level security;
alter table public.edu_conversion_reviews enable row level security;
alter table public.edu_conversion_receipts enable row level security;
revoke all on public.edu_conversion_cases,public.edu_conversion_evidence,public.edu_conversion_runs,public.edu_conversion_reviews,public.edu_conversion_receipts from public,anon,authenticated;
grant select on public.edu_conversion_cases,public.edu_conversion_evidence,public.edu_conversion_runs,public.edu_conversion_reviews to service_role;

create function public.edu_conversion_mutate(
...
begin
  ...
  -- The first pilot is low-volume. One transaction lock serializes the entire
  -- evidence candidate set, including new approvals, with snapshot validation.
  ...
  if p_action='save_case' then
    ...
    if p_action='analyze' then
      ...
      if jsonb_typeof(p_result) is distinct from 'object' or p_result->>'mode' is distinct from 'mock' or p_result->'requires_human_review' is distinct from 'true'::jsonb then raise exception 'CONVERSION_INVALID'; end if;
      -- Snapshots come from the same checked database rows, never client input.
      -- Keep them immutable so later source edits do not rewrite review history.
      ...
      insert into public.edu_conversion_runs(case_id,input_version,provider,result,evidence_versions,input_snapshot,evidence_snapshot,actor_id)
        values(inquiry.id,inquiry.input_version,'mock',p_result,versions,inquiry_snapshot,candidate_snapshot,p_actor) returning * into judgment;
      ...
      insert into public.edu_conversion_reviews(case_id,run_id,decision,reply_text,reason,actor_id)
        values(inquiry.id,judgment.id,p_payload->>'decision',p_payload->>'reply_text',p_payload->>'reason',p_actor) returning * into review_record;
