-- Count all traffic for the selected landing and period unless an explicit UTM filter is set.
-- The campaign's configured UTM identifies settings; it is not a traffic allowlist.
-- Keep RPC signatures, campaign/date boundaries, explicit filters and API-only permissions.
