-- BA-DASH-FILTER-001: expose Meta-only dimensions in dashboard filter options.
-- Keep device/layout options web-only and choose one current label per stable Meta ID.
...
meta_source as materialized (
  ...
  where (p_campaigns is null or campaign_name=any(p_campaigns)) and (p_ad_types is null or ad_type=any(p_ad_types)) and (p_adsets is null or adset_name=any(p_adsets)) and (p_creatives is null or creative_name=any(p_creatives)) and p_devices is null and p_layouts is null
...
campaign_total as (
...
), meta_campaign_options as (
 select distinct on(meta_campaign_id) campaign_name value from meta_source where period='B' and btrim(campaign_name)<>'' order by meta_campaign_id,day desc,campaign_name
), meta_adset_options as (
 select distinct on(meta_adset_id) adset_name value from meta_source where period='B' and btrim(adset_name)<>'' order by meta_adset_id,day desc,adset_name
), meta_creative_options as (
 select distinct on(meta_ad_id) creative_name value from meta_source where period='B' and btrim(creative_name)<>'' order by meta_ad_id,day desc,creative_name
), options as (
 select jsonb_build_object(
  ...
  'layouts',coalesce((select jsonb_agg(value order by value) from (select distinct layout_ver value from sessions where period='B' and layout_ver is not null)x),'[]'::jsonb)
 ) value
)
