-- BA-DASH-META-BUGFIX-001. Preserve history; only currently configured paid IDs are reportable.
alter table public.landing_campaigns add column if not exists meta_sync_attempted_at timestamptz;
...
web as (
 ...
 from selected c join public.landing_campaign_meta_daily m on c.uses_ads=true and m.campaign_id=c.id and m.meta_campaign_id=any(c.meta_campaign_ids) and m.day between p_start and p_end left join public.landing_campaign_dimensions d on d.campaign_id=m.campaign_id and d.meta_adset_id=m.meta_adset_id and d.meta_ad_id=m.meta_ad_id
