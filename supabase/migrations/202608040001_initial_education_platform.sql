...
-- UI 확인용 초기 클래스·기수·커리큘럼
insert into public.courses (
  course_code, slug, title, summary, description, category, instructor_name,
  list_price, duration_label, schedule_label, status, display_order, published_at
)
values (
  'LOCAL_MARKETING',
  'local-marketing',
  '매출을 만드는 자영업 마케팅 실전반',
  '내 매장에 맞는 고객과 매출 구조를 4주 안에 설계합니다.',
  '고객 발견부터 매출 퍼널, 콘텐츠, 광고 운영까지 실제 실행안을 완성하는 실전 클래스입니다.',
  '마케팅',
  '리키',
  490000,
  '4주',
...
insert into public.cohorts (
  course_id, cohort_code, name, note, recruitment_start_at, recruitment_end_at,
  operation_start_at, operation_end_at, price, capacity, status
)
select
  id,
  'LOCAL_MARKETING_01',
  '자영업 마케팅 실전반 1기',
  '8월 개강',
  '2026-08-04 00:00:00+09'::timestamptz,
  '2026-08-18 23:59:59+09'::timestamptz,
  '2026-08-19 20:00:00+09'::timestamptz,
  '2026-09-09 22:00:00+09'::timestamptz,
  490000,
  30,
  'recruiting'
from public.courses
where course_code = 'LOCAL_MARKETING'
on conflict (course_id, cohort_code) do nothing;

insert into public.cohort_sessions (
  cohort_id, session_number, title, description, expected_output, scheduled_at
)
select ch.id, v.session_number, v.title, v.description, v.expected_output, v.scheduled_at
from public.cohorts ch
join public.courses c on c.id = ch.course_id and c.course_code = 'LOCAL_MARKETING'
cross join (values
  (1, '고객 발견과 포지셔닝', '실제로 구매할 가능성이 높은 고객을 정의합니다.', '우리 매장 고객 정의', '2026-08-19 20:00:00+09'::timestamptz),
  (2, '매출 퍼널 설계', '유입부터 재구매까지 끊긴 구간을 찾아 연결합니다.', '매출 구조 맵', '2026-08-26 20:00:00+09'::timestamptz),
  (3, '콘텐츠 전략과 제작', '고객 행동을 이끄는 주제와 포맷을 만듭니다.', '주간 콘텐츠 캘린더', '2026-09-02 20:00:00+09'::timestamptz),
  (4, '광고 운영과 최적화', '작은 예산으로 검증하고 확장하는 광고 기준을 세웁니다.', '30일 광고 테스트 플랜', '2026-09-09 20:00:00+09'::timestamptz)
) as v(session_number, title, description, expected_output, scheduled_at)
where ch.cohort_code = 'LOCAL_MARKETING_01'
on conflict (cohort_id, session_number) do nothing;

insert into public.curriculum_weeks (
  course_id, week_number, title, goal, is_published, display_order
)
select c.id, v.week_number, v.title, v.goal, true, v.week_number * 10
from public.courses c
cross join (values
  (1, '고객 발견과 포지셔닝', '우리 매장 핵심 고객 정의'),
  (2, '매출 퍼널 설계', '유입부터 재구매까지 매출 구조 맵'),
  (3, '콘텐츠 전략과 제작', '고객 행동을 이끄는 주간 콘텐츠 캘린더'),
  (4, '광고 운영과 최적화', '30일 광고 테스트 플랜')
) as v(week_number, title, goal)
where c.course_code = 'LOCAL_MARKETING'
on conflict (course_id, week_number) do nothing;

insert into public.curriculum_lessons (
  week_id, day_number, title, description, content_type, duration_label,
  is_published, display_order
)
select
  w.id, v.day_number, v.title, v.description, v.content_type, v.duration_label,
  true, v.day_number * 10
from public.curriculum_weeks w
join public.courses c on c.id = w.course_id and c.course_code = 'LOCAL_MARKETING'
join (values
  (1, 1, '오리엔테이션 · 현재 매출 구조 진단', '4주 동안 바꿔야 할 핵심 지표와 실행 순서를 정합니다.', 'vod', '18분'),
  (1, 2, '내 고객이 아닌 사람부터 제거하기', '모두를 설득하려는 메시지에서 벗어나 핵심 고객의 범위를 좁힙니다.', 'vod', '22분'),
  (1, 3, '고객의 구매 욕구와 실제 언어 찾기', '후기와 상담 기록에서 고객이 반응하는 표현을 수집합니다.', 'material', '워크시트'),
  (2, 4, '현재 고객 여정과 이탈 구간 표시하기', '유입·문의·구매·재구매 단계의 실제 전환을 확인합니다.', 'vod', '24분'),
  (2, 5, '첫 구매를 만드는 오퍼 구조', '고객이 지금 행동해야 하는 이유와 진입 상품을 설계합니다.', 'vod', '27분'),
  (2, 6, '우리 매장 퍼널 맵 작성', '채널별 고객 행동과 다음 전환 장치를 한 장으로 정리합니다.', 'material', '템플릿'),
  (3, 7, '팔리는 콘텐츠 주제 3가지', '고객의 문제·욕구·반박을 기준으로 핵심 주제를 정합니다.', 'vod', '21분'),
  (3, 8, '채널별 콘텐츠 포맷 선택', '블로그·인스타그램·유튜브 중 전환에 맞는 형식을 고릅니다.', 'vod', '19분'),
  (3, 9, '7일 콘텐츠 캘린더 만들기', '주제와 CTA가 연결된 일주일 실행안을 완성합니다.', 'material', '템플릿'),
  (4, 10, '광고 전에 확인할 전환 기준', '클릭보다 매출에 가까운 핵심 지표와 목표값을 정합니다.', 'vod', '20분'),
  (4, 11, '작은 예산으로 광고 세팅하기', '소재·타깃·예산을 한 번에 하나씩 검증하는 구조를 만듭니다.', 'vod', '28분'),
  (4, 12, '30일 테스트 플랜 완성', '중단·유지·증액 조건이 포함된 광고 운영표를 작성합니다.', 'material', '실행표')
) as v(week_number, day_number, title, description, content_type, duration_label)
