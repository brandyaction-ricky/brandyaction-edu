begin;
-- Preserve previous revisions and the legacy RPC signature during rolling deploys.
alter table public.edu_recruitment_funnel_revisions alter column free_cohort_id drop not null;
alter table public.edu_recruitment_funnel_revisions add column flow_version integer not null default 1 check (flow_version in (1,2));

create or replace function public.edu_save_recruitment_funnel(
...
begin
  ...
  if p_request is null or p_expected is null or p_expected < 0 or p_free_course is null or p_paid_course is null
    or p_paid_cohort is null or p_free_course=p_paid_course then raise exception 'CONVERSION_INVALID'; end if;
  ...
  if found then
    if previous.expected_version<>p_expected or previous.free_course_id<>p_free_course or previous.free_cohort_id is distinct from p_free_cohort
  ...
  if not found then raise exception 'CONVERSION_NOT_FOUND'; end if;
  if p_free_cohort is not null then
  ...
  if not found then raise exception 'CONVERSION_INVALID'; end if;
  insert into public.edu_recruitment_funnel_revisions(flow_version, version, actor_id, request_id, expected_version, free_course_id, free_cohort_id, paid_course_id, paid_cohort_id)
    values(case when p_free_cohort is null then 2 else 1 end,current_version+1,p_actor,p_request,p_expected,p_free_course,p_free_cohort,p_paid_course,p_paid_cohort) returning * into previous;
  return to_jsonb(previous);
end;
$$;
