...
-- New use of Jev v4 is employee-reviewed. A subjective calibration row is no
-- longer required to create or review a judgment. Existing rows are preserved.
alter table public.edu_conversion_jev_v4_runs
  alter column calibration_review_id drop not null;
  ...
  if p_action='save_case' then
    ...
    if p_action='analyze' then
      ...
      if judgment.provider='jev' then
        -- Independent human labels are optional. Staff review and reply approval
        -- must not depend on comparing subjective labels with the model.
        if nullif(p_payload->'calibration','null'::jsonb) is null
          and nullif(p_payload->>'calibration_sample_kind','') is not null then raise exception 'CONVERSION_INVALID'; end if;
        if nullif(p_payload->'calibration','null'::jsonb) is not null
          and nullif(p_payload->>'calibration_sample_kind','') not in ('operational','test') then raise exception 'CONVERSION_INVALID'; end if;
