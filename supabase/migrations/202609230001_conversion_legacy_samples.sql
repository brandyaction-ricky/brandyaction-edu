...
-- Keep historical paid-education calibration separate from current recruitment
-- without inventing a matching course or adding old leads to funnel metrics.
alter table public.edu_conversion_cases add column sample_origin text not null default 'current';
alter table public.edu_conversion_cases add column legacy_course_label text;
alter table public.edu_conversion_cases alter column course_id drop not null;
alter table public.edu_conversion_cases add constraint edu_conversion_cases_origin_check check (
  (sample_origin='current' and course_id is not null and legacy_course_label is null)
  or (sample_origin='external_legacy' and source_type='manual' and question_id is null
    and cohort_id is null and legacy_course_label is not null
    and length(btrim(legacy_course_label)) between 1 and 200)
);
...
begin
  ...
  if p_action in ('save_case','save_evidence') then
    ...
    target_course := nullif(p_payload->>'course_id','')::uuid;
    target_cohort := nullif(p_payload->>'cohort_id','')::uuid;
    if target_course is null then
      if p_action<>'save_case' or p_payload->>'sample_origin'<>'external_legacy' then raise exception 'CONVERSION_INVALID'; end if;
    else
  ...
  if p_action='save_case' then
    ...
    if coalesce(p_payload->>'sample_origin','current') not in ('current','external_legacy') then raise exception 'CONVERSION_INVALID'; end if;
    if p_payload->>'sample_origin'='external_legacy' then
      if v_question_id is not null or target_cohort is not null or length(btrim(coalesce(p_payload->>'legacy_course_label',''))) not between 1 and 200 then raise exception 'CONVERSION_INVALID'; end if;
    elsif target_course is null or nullif(p_payload->>'legacy_course_label','') is not null then
    ...
    if target_id is not null then
      ...
      if inquiry.question_id is distinct from v_question_id or inquiry.sample_origin is distinct from coalesce(p_payload->>'sample_origin','current') then raise exception 'CONVERSION_INVALID'; end if;
      update public.edu_conversion_cases set course_id=target_course,cohort_id=target_cohort,
        sample_origin=coalesce(p_payload->>'sample_origin','current'),legacy_course_label=nullif(p_payload->>'legacy_course_label',''),
      ...
      insert into public.edu_conversion_cases(source_type,question_id,course_id,cohort_id,sample_origin,legacy_course_label,subject,content,source_label,received_at,customer_id,source_revision,actor_id)
      values(case when v_question_id is null then 'manual' else 'native' end,v_question_id,target_course,target_cohort,coalesce(p_payload->>'sample_origin','current'),nullif(p_payload->>'legacy_course_label',''),
    ...
    if p_action='analyze' then
      ...
      inquiry_snapshot := jsonb_build_object('id',inquiry.id,'source_type',inquiry.source_type,'question_id',inquiry.question_id,
        'course_id',inquiry.course_id,'cohort_id',inquiry.cohort_id,'sample_origin',inquiry.sample_origin,'legacy_course_label',inquiry.legacy_course_label,'subject',inquiry.subject,'content',inquiry.content,
