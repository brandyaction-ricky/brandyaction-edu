...
alter table public.edu_conversion_reviews
  ...
  add constraint edu_conversion_reviews_calibration_check check (
    calibration is null or (
      jsonb_typeof(calibration)='object'
      and calibration-'purchase_intent'-'primary_barrier'-'purchase_readiness'-'next_action'='{}'::jsonb
      and calibration->>'purchase_intent' in ('high','medium','low','unclear')
      and calibration->>'primary_barrier' in ('price','schedule','skill_level','content_fit','trust','none_or_unknown')
      and calibration->'purchase_readiness' in ('0'::jsonb,'1'::jsonb,'2'::jsonb,'3'::jsonb,'4'::jsonb)
      and calibration->>'next_action' in ('answer_specific_questions','invite_webinar','offer_purchase_info','human_consult','hold_no_contact')
    )
  );

create unique index edu_conversion_reviews_one_calibration_per_run
  on public.edu_conversion_reviews(run_id) where calibration is not null;
  ...
  if p_action='save_case' then
    ...
    if p_action='analyze' then
      ...
      if judgment.provider='jev' then
        ...
        if not found and nullif(p_payload->'calibration','null'::jsonb) is null then raise exception 'CONVERSION_INVALID'; end if;
        if found and nullif(p_payload->'calibration','null'::jsonb) is not null then raise exception 'CONVERSION_INVALID'; end if;
      elsif nullif(p_payload->'calibration','null'::jsonb) is not null then
        raise exception 'CONVERSION_INVALID';
      end if;
      insert into public.edu_conversion_reviews(case_id,run_id,decision,reply_text,reason,calibration,actor_id)
        values(inquiry.id,judgment.id,p_payload->>'decision',p_payload->>'reply_text',p_payload->>'reason',nullif(p_payload->'calibration','null'::jsonb),p_actor) returning * into review_record;
