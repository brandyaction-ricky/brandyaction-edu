...
const result = { contract_version: 2, model: 'jev-test', decisions: {} };
const request = (origin = 'https://edu.example') => new Request('https://edu.example/api/conversion/jev-v2', {
...
function harness(options = {}) {
  ...
  const db = { from(table) {
    ...
    const query = {
      ...
      async maybeSingle() {
        ...
        if (table === 'edu_conversion_jev_v2_runs') return { data: stored, error: null };
  ...
  const handlers = load('../app/api/conversion/jev-v2/route.ts', {
    ...
    '@/lib/conversion-jev-v2': { createJevV2Judgment: async (...args) => { events.push('provider'); assert.deepEqual(args.slice(0, 2), ['당시 문의', '당시 원문']); if (options.providerFails) throw new Error('private upstream response'); return result; } },
...
test('v2 requires DEV, same origin, and operator before accessing inquiry data', async () => {
...
test('v2 only sends the unchanged v1 snapshot with an operational first review and stores separately', async () => {
  const h = harness();
  ...
  assert.equal((await response.json()).result.contract_version, 2);
...
test('completed v2 result is idempotently returned without a second provider call', async () => {
