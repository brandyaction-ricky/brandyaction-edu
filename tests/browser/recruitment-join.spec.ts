...
test('join screen has a keyboard accessible explicit action and no automatic navigation',async({page,baseURL})=>{
 let posts=0;
 await page.route('**/join/synthetic/paid',async route=>{
  if(route.request().method()==='POST'){posts++;expect(new URLSearchParams(route.request().postData()!).get('version')).toBe('1');expect(route.request().headers().origin).toBe(new URL(baseURL!).origin);return route.fulfill({headers:RECRUITMENT_JOIN_HEADERS,contentType:'text/html; charset=utf-8',body:'<h1>합성 이동 확인</h1>'});}
