...
export function ProductSaleFixture() {
  ...
  return <div className="edu-admin" style={{ padding: 20 }}><output aria-label="합성 저장 횟수">{saves}</output><ProductEditor row={course} data={data} pending={false} back={() => {}} send={async () => { setSaves(value => value + 1); return {}; }} /></div>;
}
