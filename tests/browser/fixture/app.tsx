...
createRoot(document.getElementById('root')!).render(<StrictMode>{path === '/product-countdown-test' ? <ProductCountdownFixture /> : new URLSearchParams(location.search).has('navigationFixture') ? <AdminWorkspace>{null}</AdminWorkspace> : path === '/product-sale-test' ? <ProductSaleFixture /> : fixture}</StrictMode>);
