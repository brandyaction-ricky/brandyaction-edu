...
export function MissionFixture() {
  ...
  const send: WorkflowSend=async body=>{
    setPending(true);try{
      ...
      if(body.action==='save') {
        ...
        if(body.draft){const draft={id:'synthetic-draft',enrollment_id:enrollment,mission_id:target.id,content:body.content,url:body.url,response,updated_at:new Date().toISOString()};store({...data,edu_mission_drafts:[...data.edu_mission_drafts.filter(d=>d.mission_id!==target.id),draft]});}
  ...
  return <div className={['member','perform'].includes(mode)?'edu-front':'edu-admin'} style={{maxWidth:1208,margin:'0 auto',padding:24,minHeight:'100vh'}}>
    ...
    {mode==='review'&&<><h1>제출물 검토</h1><SubmissionReview data={data} send={send} pending={pending}/></>}
