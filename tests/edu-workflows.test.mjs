...
test('measurement data accepts live peak, staged payments and memo',()=>{
 const value=rules.validateSetting('metrics',{date:'2026-09-14',campaign:'live',spend:1000,impressions:100,clicks:10,leads:4,revenue:30000,livePeak:57,paymentsDay0:2,paymentsDay1:1,paymentsDay2:0,paymentsDay3:1,paymentsDay4:0,memo:'라이브 종료 후 집계'});
 assert.equal(value.livePeak,57);assert.equal(value.paymentsDay3,1);assert.equal(value.memo,'라이브 종료 후 집계');
 assert.throws(()=>rules.validateSetting('metrics',{...value,paymentsDay4:-1}));
});
